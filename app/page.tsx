...
import { CapabilityCard, TierBody, SandboxCard, ConsoleCard } from '@arf/ui';
...
const TIERS = [
  ...
  },
  {
    ...
    price: 'By review',
    items: ['Protected core access', 'Outcome-based or retainer after pilot', 'Founder-led onboarding'],
...
const FOOTER_COLUMNS = [
  ...
  },
  {
    ...
    links: [
      ...
      { label: 'LinkedIn', href: 'https://www.linkedin.com/company/agentic-reliability' },
  ...
  },
  {
    ...
    links: [
      ...
      { label: 'Risk demo', href: 'https://arf-foundation.github.io/arf-risk-demo/' },
    ],
  },
] as const;
...
export default function LandingPage() {
  ...
  const handleCopy = async () => {
    try {
      ...
      setCopied(true);
      timers.current.push(setTimeout(() => isMounted.current && setCopied(false), 2000));
    } catch (err) {
      console.warn('Clipboard copy failed:', err);
      setCopyError('Could not copy the curl command');
      timers.current.push(setTimeout(() => isMounted.current && setCopyError(null), 2000));
    }
  };
  ...
  const { ref: capsRef, inView: capsInView } = useInView({ threshold: 0.15, once: true });
  const { ref: govRef, inView: govInView } = useInView({ threshold: 0.15, once: true });
  const { ref: pricingRef, inView: pricingInView } = useInView({ threshold: 0.15, once: true });
    ...
    <div className="arf-page-root">
      ...
      <section className="arf-shell pb-[112px]">
        <div className="arf-card grid md:grid-cols-3">
          {[
            ...
            >
              <p className={`mb-3.5 font-mono text-[11px] font-medium uppercase tracking-[0.14em] ${item.tone}`}>
                {item.label}
              </p>
              <p className="text-[17px] leading-[1.55] text-pretty">{item.body}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="arf-shell grid gap-16 pb-24 lg:grid-cols-[0.9fr_1.1fr]">
        ...
        <div>
          <p className="mb-5 text-[21px] font-semibold leading-[1.45] tracking-[-0.014em] text-pretty">
            Foundation models are probabilistic. Enterprise operations require deterministic control. ARF bridges that
            gap.
          </p>
          <p className="text-body text-[color:var(--text-secondary)] text-pretty">
            Autonomous AI promises unprecedented speed and scale, but without governance it introduces unacceptable
            operational risk. ARF provides the missing control plane — translating probabilistic model outputs into
            verifiable, auditable actions that align with your business policies.
          </p>
        </div>
      </section>
        ...
        <div className="grid gap-16 rounded-[18px] border border-arf-blue/15 bg-gradient-to-br from-arf-blue/10 to-arf-purple/10 px-14 py-[60px] lg:grid-cols-[0.9fr_1.1fr]">
          ...
          <div>
            <p className="mb-4.5 text-body text-[color:var(--text-primary)]/85 text-pretty">
              Autonomous AI is moving from copilots to autonomous workflows. As AI gains the ability to act — not just
              recommend — organisations need infrastructure that governs execution, manages operational risk, and
              provides auditability by design.
            </p>
            <p className="text-body text-[color:var(--text-secondary)] text-pretty">
              ARF AI delivers that control plane. Built for the era where AI doesn&rsquo;t just answer questions — it
              deploys code, modifies infrastructure, and makes decisions that affect business outcomes.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="arf-shell pb-[120px]">
        <div className="mb-7 flex flex-wrap items-baseline justify-between gap-6">
          <h2 className="text-[27px] font-semibold tracking-[-0.022em]">Built for regulated enterprises</h2>
        ...
        <div className="flex flex-wrap gap-2.5">
          {INDUSTRIES.map((ind) => (
            ...
            >
              ...
              <span className="text-sm font-semibold tracking-[-0.01em]">{ind.name}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      >
        <div className="mb-10 grid gap-16 lg:grid-cols-[0.8fr_1.2fr]">
          ...
          <p className="max-w-[56ch] self-end text-base leading-[1.65] text-[color:var(--text-secondary)] text-pretty">
            Four subsystems, one control plane. Every capability is observable from the Governance Console and
            enforceable from the API.
          </p>
        </div>
      ...
      >
        <div className="arf-shell">
          <div className="mb-11 grid gap-16 lg:grid-cols-[0.85fr_1.15fr]">
            <h2 className="max-w-[14ch] text-h2-lg font-semibold">Enterprise-grade governance</h2>
            <p className="max-w-[52ch] self-end text-body text-[color:var(--text-secondary)] text-pretty">
              The three properties a compliance officer will ask about in the first meeting. Each one is a mechanism,
              not a policy document.
            </p>
          </div>
          <div className="grid gap-[22px] md:grid-cols-3">
            {GOVERNANCE.map((item) => (
              <div key={item.title} className="arf-card-substantial bg-[color:var(--surface-canvas)] p-9">
                ...
                <h3 className="mb-3 text-[19px] font-semibold tracking-[-0.016em]">{item.title}</h3>
                <p className="text-[15px] leading-[1.6] text-[color:var(--text-secondary)]">{item.body}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="arf-shell grid items-center gap-16 lg:grid-cols-[1.15fr_0.85fr]">
          <div>
            ...
            <blockquote className="mb-7 font-serif text-[clamp(1.75rem,3vw,2.125rem)] font-light italic leading-[1.35] tracking-[-0.01em] text-white text-pretty">
              &ldquo;We could not put agents anywhere near production until every action had a gate in front of it and a
              record behind it. ARF gave our risk committee something they could actually read.&rdquo;
            </blockquote>
            <div className="flex items-center gap-3.5">
              ...
              />
              <div>
                <p className="text-[14.5px] font-semibold text-white">Head of AI Platform</p>
                <p className="text-[13.5px] text-white/65">Tier-1 financial services · pilot organisation</p>
              </div>
            </div>
          </div>
          <div className="rounded-2xl border border-white/15 bg-white/[0.06] p-8">
            ...
            <dl className="flex flex-col gap-5">
              {[
                { value: '100%', label: 'of autonomous actions gated and recorded' },
                { value: '42ms', label: 'median policy evaluation overhead' },
                { value: '0', label: 'silent overrides — every exception is signed' },
              ].map((stat, idx) => (
                <div key={stat.value} className={idx > 0 ? 'border-t border-white/15 pt-5' : ''}>
                  <dt className="mb-1 text-[32px] font-semibold leading-none tracking-[-0.028em] text-white">
                    {stat.value}
                  </dt>
                  <dd className="text-[13.5px] leading-[1.5] text-white/70">{stat.label}</dd>
                </div>
              ))}
            </dl>
          </div>
      ...
      <section id="explore" className="arf-shell pb-[104px]">
        <div className="mb-8 flex flex-wrap items-baseline justify-between gap-8">
          ...
          <p className="max-w-[50ch] text-[13.5px] leading-[1.6] text-[color:var(--text-secondary)] lg:text-right">
            The public sandbox returns simulated responses only. Real enforcement and confidence guarantees require a
            pilot agreement.
          </p>
        </div>
        <div className="grid gap-[22px] lg:grid-cols-[1.2fr_0.9fr_0.9fr]">
          ...
          <div className="arf-card-light flex flex-col p-7">
            ...
            <Link
              href="/signup"
              className="mt-auto inline-flex items-center gap-2 text-[14.5px] font-semibold text-arf-blue"
            >
      ...
      <footer className="bg-arf-dark px-0 pb-8 pt-[72px] text-white/70">
        <div className="arf-shell">
          <div className="grid gap-11 border-b border-white/12 pb-[52px] lg:grid-cols-[1.4fr_repeat(4,1fr)]">
            <div>
              ...
              <p className="mb-6 max-w-[34ch] text-sm leading-[1.65]">
                The control plane between autonomous AI and enterprise infrastructure.
              </p>
              <a
                  ...
                  {col.links.map((link) =>
                    link.href.startsWith('http') ? (
                      <a
                        ...
                        onClick={link.label === 'Slack' ? trackSlackClick : undefined}
          ...
          <div className="flex flex-wrap items-center justify-between gap-8 pt-7">
            <a href="https://github.com/arf-foundation" target="_blank" rel="noopener noreferrer" className="opacity-80">
              <Image src="/GitHub_Lockup_White.svg" alt="GitHub Enterprise" width={120} height={34} className="h-8 w-auto" />
            </a>
